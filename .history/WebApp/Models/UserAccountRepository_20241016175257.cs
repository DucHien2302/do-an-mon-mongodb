...
public class UserAccountRepository
{
    ...
    public async Task DeleteEducationDetailByDegreeAsync(ObjectId id, string DegreeName)
    {
        var collection = connection.GetCollection<UserAccount>("user_account");
        var filter = Builders<UserAccount>.Filter.Eq(u => u.Id, id);
        var update = Builders<UserAccount>.Update.PullFilter(u => u.EducationDetails, e => e.CertificateDegreeName == DegreeName);
        var result = await collection.UpdateOneAsync(filter, update);
    }
}