...
public class UserAccountRepository
{
    ...
    public async Task DeleteExperienceDetailTheoJobTitleAsync(string userId, string jobTitle)
    {
        // Tạo bộ lọc tìm kiếm tài liệu UserAccount theo _id
        var filter = Builders<UserAccount>.Filter.Eq("_id", userId);
        ...
        {
            Console.WriteLine("Không tìm thấy tài liệu hoặc không có ExperienceDetail khớp với JobTitle đã chỉ định.");
        }
        else
        {
            Console.WriteLine($"ExperienceDetail với JobTitle '{jobTitle}' đã được xóa thành công.");
        }
    }
