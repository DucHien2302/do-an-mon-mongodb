...
public class UserAccountController : Controller
{
    ...
    [HttpPost]
    public async IActionResult Update(UserAccount obj, IFormFile f)
    {
        if (f != null)
        {
        ...
        UserAccount user = await userAccountRepository.GetUserAccount(obj.Id);
        if (user != null)
        {
            var ret = userAccountRepository.UpdateUserAccountAsync(obj.Id, obj);
            if ()
        }
